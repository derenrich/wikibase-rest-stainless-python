...
def test_response_parse_bool(client: WikibaseRestStainless, content: str, expected: bool) -> None:
    response = APIResponse(
...
async def test_async_response_parse_bool(client: AsyncWikibaseRestStainless, content: str, expected: bool) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=content),
        client=client,
        stream=False,
...
def test_response_parse_expect_model_union_non_json_content(client: WikibaseRestStainless) -> None:
    response = APIResponse(
