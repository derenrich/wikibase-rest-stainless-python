...
from ._exceptions import APIResponseValidationError, PyWikibaseRestStainlessError
...
class BaseAPIResponse(Generic[R]):
    ...
    def _parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        if inspect.isclass(origin) and not issubclass(origin, BaseModel) and issubclass(origin, pydantic.BaseModel):
            raise TypeError(
                "Pydantic models must subclass our base model type, e.g. `from py_wikibase_rest_stainless import BaseModel`"
            )
...
class APIResponse(BaseAPIResponse[R]):
    ...
    def parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        ```py
        from py_wikibase_rest_stainless import BaseModel
...
class MissingStreamClassError(TypeError):
    def __init__(self) -> None:
        super().__init__(
            "The `stream` argument was set to `True` but the `stream_cls` argument was not given. See `py_wikibase_rest_stainless._streaming` for reference",
        )


class StreamAlreadyConsumed(PyWikibaseRestStainlessError):
    """
